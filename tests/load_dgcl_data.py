...
def load_dgcl_file(path=path_assets + "/data/2021-communes-criteres-repartition.csv"):
    try:
...
if __name__ == "__main__":
    filename = path_assets + "/data/2021-communes-criteres-repartition.csv"
    data = adapt_dgcl_data(load_dgcl_file(filename))
    out_file = f'{Path(filename).parent.resolve()}/{Path(filename).resolve().stem}-loaded.csv'
    print(f'Saving to {out_file}')
    data.to_csv(out_file, index=False, decimal=",", sep=';')
    print(data)
